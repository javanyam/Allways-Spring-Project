		...
		<table>
			<tr align="center" height="20">
				<td>
					...
					<c:if test="${index != 1}">
						<a href="customerOrders?index=${index-1}">이전</a>&nbsp;
					</c:if> 
					...
					<c:forEach var="cnt" begin="${pagecount * pagepage + 1}" end="${pagecount * (pagepage + 1)}">
						<c:if test="${cnt <= Math.ceil(arrsize / rowcount)}">
							...
							<c:if test="${cnt == index }">
								<a href="customerOrders?index=${cnt }" style="font-size:1.3em">[${cnt}]</a>
							</c:if>
					
							<c:if test = "${cnt != index }">
								<a href="customerOrders?index=${cnt }" style="font-size:0.9em">[${cnt}]</a>&nbsp;
							</c:if>
					...
					<c:if test="${index < Math.ceil(arrsize / rowcount)}">
						<a href="customerOrders?index=${index+1 }">다음</a>&nbsp;
					</c:if>
